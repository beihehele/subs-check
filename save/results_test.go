...
func TestSaveConfig_SnapshotMatchesSubscription(t *testing.T) {
	for _, rename := range []bool{false, true} {
		name := "original names"
		if rename {
			name = "region sequence names"
		}
		t.Run(name, func(t *testing.T) {
			...
			*config.GlobalConfig = config.Config{
				OutputDir: dir, SaveMethod: "local", RenameNode: rename,
				NodePrefix: "SC-", SpeedTestUrl: "https://example.com/speed",
			}
			ResultsPath = func() string { return filepath.Join(dir, "results.json") }
			results := []check.Result{
				{Proxy: map[string]any{"name": "slow", "sub_tag": "source"}, Country: "HK", Speed: 100},
				{Proxy: map[string]any{"name": "japan"}, Country: "JP", Speed: 200},
				{Proxy: map[string]any{"name": "fast"}, Country: "HK", Speed: 150},
			}
			SaveConfig(results)
			...
			if err := yaml.Unmarshal(data, &subscription); err != nil {
				t.Fatal(err)
			}
			...
			want := []string{"fast|150KB/s", "slow|100KB/s|source", "japan|200KB/s"}
			if rename {
				want = []string{"SC-🇭🇰HK_1|150KB/s", "SC-🇭🇰HK_2|100KB/s|source", "SC-🇯🇵JP_1|200KB/s"}
			}
			if len(snapshot.Nodes) != len(want) || len(subscription.Proxies) != len(want) {
				t.Fatalf("node counts: snapshot=%d subscription=%d", len(snapshot.Nodes), len(subscription.Proxies))
			}
			for i, name := range want {
				if snapshot.Nodes[i].Name != name || subscription.Proxies[i]["name"] != name {
					t.Errorf("node %d: snapshot=%q subscription=%q want=%q", i, snapshot.Nodes[i].Name, subscription.Proxies[i]["name"], name)
				}
				if snapshot.Nodes[i].BaseName != strings.Split(name, "|")[0] {
					t.Errorf("node %d: base name = %q", i, snapshot.Nodes[i].BaseName)
				}
			}
		})
	}
